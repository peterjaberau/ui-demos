...
import { BoardProps, ItemsPaletteProps } from "@cloudscape-design/board-components";
// @ts-ignore
import { fromMatrix } from '@/cloudscape/internals/board/debug-tools';
// @ts-ignore
import { BoardItemDefinition, BoardItemDefinitionBase } from '@cloudscape-design/board-components/internal/interfaces';
...
function applyLayout<D>(
  ...
  const sortedLayout = layout.items.slice().sort((a: any, b: any) => {
    if (a.y !== b.y) {
      return a.y > b.y ? 1 : -1;
    }
    return a.x > b.x ? 1 : -1;
  ...
  return sortedLayout.map(({ id, x, width, height }: any) => ({
    ...getItem(id),
